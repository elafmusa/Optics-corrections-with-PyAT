...
def generatingQuadsResponse(ring, C0x, C0y, C0xy, C0yx, correctrs_kick,used_cor_indexes, quads_indexes, dk, debug=False):
    ...
    for quad_index in quads_indexes:
        ...
        if ring[quad_index].FamName.startswith('s') or ring[quad_index].FamName.startswith('S'):
            C1x, C1xy, C1y, C1yx = quadsSensitivityMatrices_skew(ring,correctrs_kick,used_cor_indexes,  quad_index, dk)
        else:
...
def generatingQuadsResponse_skew(ring, C0x, C0y, C0xy, C0yx, correctrs_kick,used_cor_indexes, quads_indexes, dk, debug=False):
    dCx = []
    dCy = []
    dCxy = []
    dCyx = []
...
def quadsSensitivityMatrices_skew(ring,correctrs_kick,used_cor_indexes,  quad_index, dk):
    strength_before = ring[quad_index].PolynomA[1]
    ring[quad_index].PolynomA[1] = strength_before + dk
    qxx, qxy, qyy, qyx = ORMs(correctrs_kick, ring,used_cor_indexes)
    ring[quad_index].PolynomA[1] = strength_before
