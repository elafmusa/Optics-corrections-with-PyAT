...
def ORM_x(dkick, ring, used_correctors_names):
    Cxx = []
    Cxy = []
    bpm_indexes = get_refpts(ring, at.elements.Monitor)
    _, _, elemdata = at.get_optics(ring, bpm_indexes)
    ...
    for i in range(len(used_correctors_names)):
        ...
        ring[cor_index].KickAngle[0] = dkick + a
        _, _, elemdata = at.get_optics(ring, bpm_indexes)
        closed_orbitx = elemdata.closed_orbit[:, 0] - closed_orbitx0
        closed_orbity = elemdata.closed_orbit[:, 2] - closed_orbity0
        Cxx.append(closed_orbitx)
...
def ORM_y(dkick, ring, used_correctors_names):
    Cyy = []
    Cyx = []
    bpm_indexes = get_refpts(ring, at.elements.Monitor)
    _, _, elemdata = at.get_optics(ring, bpm_indexes)
    ...
    for i in range(len(used_correctors_names)):
        ...
        ring[cor_index].KickAngle[1] = dkick + a
        _, _, elemdata = at.get_optics(ring, bpm_indexes)
        closed_orbitx = elemdata.closed_orbit[:, 0] - closed_orbitx0
        closed_orbity = elemdata.closed_orbit[:, 2] - closed_orbity0
        Cyy.append(closed_orbity)
...
def quadsSensitivityMatrices(ring,correctors_kick,used_cor_indexes,  quad_index, dk):
...
def generatingQuadsResponse_(ring, C0x, C0y, C0xy, C0yx, correctors_kick,used_cor_h,used_cor_v, quads_indexes, dk, debug=False):
    dCx = []
    dCy = []
    dCxy = []
    dCyx = []
    for quad_index in quads_indexes:
        if debug == True :
           print('generating response to quad of index', quad_index)
        C1x, C1xy, C1y, C1yx = quadsSensitivityMatrices_(ring,correctors_kick,used_cor_h,used_cor_v,  quad_index, dk)
        dCx.append((C1x - C0x) / dk)
        dCy.append((C1y - C0y) / dk)
        dCxy.append((C1xy - C0xy) / dk)
        dCyx.append((C1yx - C0yx) / dk)
...
def quadsSensitivityMatrices_(ring,correctors_kick,used_cor_h,used_cor_v,  quad_index, dk):
    strength_before = ring[quad_index].K
    ring[quad_index].K = strength_before + dk
    qxx, qxy = ORM_x(correctors_kick, ring, used_cor_h)
    qyy, qyx = ORM_y(correctors_kick, ring, used_cor_v)
...
def defineJacobianMatrices_weighted(W, C0x, C0y, Cxx_err, Cyy_err, dCx, dCy):
    Nk = len(dCx)  # number of free parameters
...
def dynamicAperture(ring, eh, ev, dr, dp, nturns, xp0, yp0,fileName,showPlot=False, debug=False):
    ...
    while (theta < 180):
        ...
        theta += 10
    import pickle
    with open("da_x_{}.bin".format(fileName), "wb") as output:
        pickle.dump(x_a, output)

    with open("da_y_{}.bin".format(fileName), "wb") as output:
        pickle.dump(y_a, output)
...
def dynamicAperture_(ring, theta_steps, dr,dr2, dp, nturns, xp0, yp0,fileName,showPlot=False, debug=False):
    ...
    while (theta < 180):
        if debug == True:
           print(f"theta={theta}")
        r0 = 0
        r = find_line_DA_(ring, theta, r0, dr, dp, nturns, xp0, yp0
                               )
        max_value_x = 0.0
        max_value_y = 0.0
        b = r - dr
        if r > r0 + tol:
            r_1 = find_line_DA_(ring, theta, b, dr2, dp, nturns, xp0, yp0
                                    )
        else:
            r_1 = r0
...
def find_line_DA_(ring, theta, r0, dr, dp, nturns, xp0, yp0):
    max_value_x = 0.0
    max_value_y = 0.0
    radius = r0
    while (not np.isnan(max_value_x) and not np.isnan(max_value_y)):

        x0 = radius * cos(np.radians(theta))
        y0 = radius * sin(np.radians(theta))
        pin1 = [np.array([x0]), np.array([xp0])]
        pin2 = [np.array([y0]), np.array([yp0])]
        pin3 = [np.array([dp])]
        new_pin = np.concatenate((pin1, pin2, pin3, [np.zeros(1)]))
